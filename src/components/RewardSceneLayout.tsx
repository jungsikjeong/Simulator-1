...
export default function RewardSceneLayout({ images, bgColor = 'bg-yellow-50', borderColor = 'border-yellow-300', textColor = 'text-yellow-700', sceneText, guideText = '아래로 스와이프(또는 클릭) 시 다운로드' }: RewardSceneLayoutProps) {
    return (
        <div className={`flex flex-col items-center gap-8 py-10 ${bgColor} min-h-screen overflow-x-auto`}>
            {sceneText && (
                <div className={`mb-6 text-2xl font-bold text-center ${textColor} drop-shadow-sm`}>{sceneText}</div>
            )}
                ...
                {images.map(({ src, label }) => (
                    ...
                    >
                        <img
                            src={src}
                            alt={label}
                            className={`w-full max-w-full h-auto rounded-xl shadow-lg select-none border-6 ${borderColor}`}
                            draggable={false}
                        />
                        <div className={`mt-2 text-center font-bold text-lg ${textColor}`}>{label}</div>
